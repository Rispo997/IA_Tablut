...
public class TablutIAClient extends TablutClient {
	...
	public TablutIAClient(String player, String name, int gameChosen) throws UnknownHostException, IOException {
		super(player, name);
		game = gameChosen;
	...
	public TablutIAClient(String player) throws UnknownHostException, IOException {
		this(player, "random", 4);
	}

	public TablutIAClient(String player, String name) throws UnknownHostException, IOException {
		this(player, name, 4);
	}

	public TablutIAClient(String player, int gameChosen) throws UnknownHostException, IOException {
	...
	private Map<int, int> bestMove(State gameState, Map<String, List<String>> possibleMoves, String ) {
		...
		for (int i = 0; i < gameState.getBoard().length; i++) {
			for (int j = 0; j < gameState.getBoard().length; j++) {
				if (gameState.getPawn(i, j).equalsPawn(State.Pawn.BLACK.toString())) {
					state[i*gameState.getBoard().length + j * 3] = 1;
					state[i*gameState.getBoard().length + j * 3 + 1] = 0;
					state[i*gameState.getBoard().length + j * 3 + 2] = 0;
				} else if (gameState.getPawn(i, j).equalsPawn(State.Pawn.WHITE.toString())) {
					state[i*gameState.getBoard().length + j * 3] = 0;
					state[i*gameState.getBoard().length + j * 3 + 1] = 1;
					state[i*gameState.getBoard().length + j * 3 + 2] = 0;
				} else if (gameState.getPawn(i, j).equalsPawn(State.Pawn.KING.toString())) {
					state[i*gameState.getBoard().length + j * 3] = 0;
					state[i*gameState.getBoard().length + j * 3 + 1] = 0;
					state[i*gameState.getBoard().length + j * 3 + 2] = 1;
				} else {
					state[i*gameState.getBoard().length + j * 3] = 0;
					state[i*gameState.getBoard().length + j * 3 + 1] = 0;
					state[i*gameState.getBoard().length + j * 3 + 2] = 0;
		...
		for(String from : possibleMoves.keySet()) {
			for(String to : possibleMoves.get(from)) {
				state[from*gameState.getBoard().length + to * 3]
				Model.score()
			}
		}
	}
		...
		while (true) {
			...
			if (this.getPlayer().equals(Turn.WHITE)) {
				...
				if (this.getCurrentState().getTurn().equals(StateTablut.Turn.WHITE)) {
					...
					try {
						possibleMoves = getPossibleMoves(pawns, state, rules, Turn.WHITE);
