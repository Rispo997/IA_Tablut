...
public class TablutIAClient extends TablutClient {
	...
	public static void main(String[] args) throws UnknownHostException, IOException, ClassNotFoundException {
		...
		TablutIAClient client = new TablutIAClient(role, name, gametype);
		client.run();
	}
	...
	private Action getBestMove(Game rules, State gameState, Map<String, List<String>> possibleMoves, Turn player) {
		...
		Action act = null;
		try {
			act = new Action(bestFrom, bestTo, player);
			if (first) {
				first = false;
				System.out.println("Stato: [");
				for(int i = 0; i < currentState.length-1; i+=3) {
					if(currentState[i] == 1)
						System.out.print("B ");
		...
		while (true) {
			...
			if (this.getPlayer().equals(Turn.WHITE)) {
				...
				if (this.getCurrentState().getTurn().equals(StateTablut.Turn.WHITE)) {
					...
					int[] selected = null;
					Map<String, List<String>> possibleMoves = null;
					try {
					...
					while (!found) {
						/*
						 * if (pawns.size() > 1) { selected = pawns.get(new
						 * Random().nextInt(pawns.size() - 1)); } else { selected = pawns.get(0); }
						 * 
						 * String from = this.getCurrentState().getBox(selected[0], selected[1]);
						 * 
						 * selected = empty.get(new Random().nextInt(empty.size() - 1)); String to =
						 * this.getCurrentState().getBox(selected[0], selected[1]);
						 */

						a = getBestMove(rules, state, possibleMoves, this.getPlayer());
